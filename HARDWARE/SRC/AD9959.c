...
void _Init_AD9959_GPIO(void)
{
    ...
    // 初始化 SDIO0
    GPIO_InitStruct.Pin = SDIO0.Pin;
    HAL_GPIO_Init(SDIO0.GPIOx, &GPIO_InitStruct);
    
    // 初始化 SDIO1
    GPIO_InitStruct.Pin = SDIO1.Pin;
    HAL_GPIO_Init(SDIO1.GPIOx, &GPIO_InitStruct);
    
    // 初始化 SDIO2
    GPIO_InitStruct.Pin = SDIO2.Pin;
    HAL_GPIO_Init(SDIO2.GPIOx, &GPIO_InitStruct);
    
    // 初始化 SDIO3
    GPIO_InitStruct.Pin = SDIO3.Pin;
    HAL_GPIO_Init(SDIO3.GPIOx, &GPIO_InitStruct);
    
    // 初始化 PS0
    GPIO_InitStruct.Pin = PS0.Pin;
    HAL_GPIO_Init(PS0.GPIOx, &GPIO_InitStruct);
    
    // 初始化 PS1
    GPIO_InitStruct.Pin = PS1.Pin;
    HAL_GPIO_Init(PS1.GPIOx, &GPIO_InitStruct);
    
    // 初始化 PS2
    GPIO_InitStruct.Pin = PS2.Pin;
    HAL_GPIO_Init(PS2.GPIOx, &GPIO_InitStruct);
    
    // 初始化 PS3
    GPIO_InitStruct.Pin = PS3.Pin;
    HAL_GPIO_Init(PS3.GPIOx, &GPIO_InitStruct);
    
    // 初始化 PDC
    GPIO_InitStruct.Pin = PDC.Pin;
    HAL_GPIO_Init(PDC.GPIOx, &GPIO_InitStruct);
    
    // 初始化 RST
    GPIO_InitStruct.Pin = RST.Pin;
    HAL_GPIO_Init(RST.GPIOx, &GPIO_InitStruct);
    
    // 初始化 UPDATE
    GPIO_InitStruct.Pin = UPDATE.Pin;
    HAL_GPIO_Init(UPDATE.GPIOx, &GPIO_InitStruct);
    ...
    // 初始化 SCLK
    GPIO_InitStruct.Pin = SCLK.Pin;
    HAL_GPIO_Init(SCLK.GPIOx, &GPIO_InitStruct);
}
...
/**
 * @brief Writes the frequency value strictly according to External Clock frequency(synchronize the output to clock, which could be external signal)
 *
 * @param Channel The channel number to write the frequency to.(0 to 3)
 * @param exponential used to divide the main CLK on dds by 2^exponential. 4<=exponential<=32
 * @param multiplier used to multiply the frequency. multiplier should be less than 2^(exponential-4)
 * for example, if your external clock for dds is 25MHz, and you want output 18.75MHz, which is 3/4 of 25MHz, then you should set exponential=6 and multiplier=3.
 */
void Synchronize_Frequence(uint8_t Channel, uint8_t exponential, uint32_t multiplier)
{
    uint8_t CFTW0_DATA[4] = {0};

    if (exponential<4 || exponential>32)
    {
        Freq2Word(14434, CFTW0_DATA);
        AD9959_error();        
    }
    else 
    {
        uint32_t freq_word=(0x80000000>>(exponential-1))*multiplier;
        CFTW0_DATA[3]=freq_word&0xff;
        CFTW0_DATA[2]=(freq_word>>8)&0xff;
        CFTW0_DATA[1]=(freq_word>>16)&0xff;
        CFTW0_DATA[0]=(freq_word>>24)&0xff;
    }
    Channel_Select(Channel);
...
void SET_2ASK(uint8_t Channel, double f, uint16_t A_start, uint16_t A_stop)
{
    ...
    uint32_t amp_word = (uint32_t)A_stop << 22;  // 10位移至高位
    A_stopWord[0] = amp_word >> 24;
    A_stopWord[1] = amp_word >> 16;
    A_stopWord[2] = amp_word >> 8;
    A_stopWord[3] = amp_word;
